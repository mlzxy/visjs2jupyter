...
Javascript(
    "$.getScript('https://cdnjs.cloudflare.com/ajax/libs/vis/4.21.0/vis.js')")
...
def visjs_network(nodes_dict, edges_dict,
                  ...
                  # back-up image in case a node image doesn't successfully load
                  node_broken_image='undefined',
                  ...
                  node_color_hover_border='#2B7CE9',
                  # color of node when mouse hovers but does not click
                  node_color_hover_background='orange',
                  ...
                  node_scaling_label_max_visible=30,
                  ...
                  # creates a directed edge with arrow head coming from delivering node
                  ...
                  # 'horizontal', 'vertical', and 'none'. Only for cubicBezier curves
                  edge_smooth_force_direction='none',
    ...
    if output == "jupyter":
        html_return = HTML(
            ...
            + 'function setUpFrame() { '
            + '    var frame = window.frames["' + fname_temp.replace(
                '.html', '').replace('html/', '') + '"];'
            + '    frame.runVis(' + dumps(nodes_dict) +
            ', ' + dumps(edges_dict) + ');'
            + '}'
            + '</script>'
            + '<iframe name="' +
            fname_temp.replace('.html', '').replace('html/', '')
            + '" src="' + fname_temp + '" width="100%;" height="' +
            str(graph_height + 5) + 'px"></iframe>'
            + '</body>'
            + '</html>'
        )
        return html_return
...
def export_to_cytoscape(nodes_dict=0,
    ...
    if (G == 0):
        ...
        nodes = [node['id']
                 for node in nodes_dict]  # nodes_dict must contain id
        # relabel ids so they match the edges
        ...
        nx.set_node_attributes(G, 'node_name', dict(zip(range(len(nodes)), [
                               str(n) for n in nodes])))  # make sure node names are strings
        ...
        for i in range(len(edges_no_data)):
            if G.is_multigraph():
                edges_dict.append(
                    {"source": edges_no_data[i][0], "target": edges_no_data[i][1], "key": edges_no_data[i][2]})
            else:
                edges_dict.append(
                    {"source": edges_no_data[i][0], "target": edges_no_data[i][1]})
...
#            edges_dict.append(edge[2])
        ...
        if G.is_multigraph():
            # edges_dict must contain source and target
            edges = [(edge['source'], edge['target'], edge['key'])
    ...
    # under the assumption that all nodes have the same attributes!!!!
    for attribute in nodes_dict[0].keys():
        ...
        if attribute == 'x':
            ...
            border_width = {node['id']: node['border_width']
...
#        nx.set_node_attributes(G, name = 'name', values = node_name)
    ...
    # assuming edges_dict has a source attribute
    sources = [str(edge['source']) for edge in edges_dict]
    # assuming edges_dict has a target attribute
    targets = [str(edge['target']) for edge in edges_dict]
    ...
    for j in range(len(G.edges())):
        to_string += str('{\"data\":')
        # 2 correspondes to the edge attributes
        to_string += str(list(G.edges(data=True))[j][2])
        if (j == len(G.edges()) - 1):
            to_string += str('}]},\"data\": {}}')
        else:
            to_string += str('},')
...
def return_node_to_color(G, field_to_map='degree', cmap=mpl.cm.jet, alpha=1.0, color_vals_transform=None, ceil_val=10,
    ...
    if color_vals_transform == 'log':
        ...
        # set the zero d values to minimum non0 value
        data = [np.log(np.max([d, min_dn0])) for d in data]
        # shift so we don't have any negative values
        data = [(d-np.nanmin(data)) for d in data]
...
def return_edge_to_color(G, field_to_map='degree', cmap=mpl.cm.jet, alpha=1.0, color_vals_transform=None, ceil_val=10,
    ...
    if (type(G) == nx.classes.multigraph.MultiGraph) | (type(G) == nx.classes.multidigraph.MultiDiGraph):
        ...
        edges_with_data = [(e[0], e[1], e[2], e[3][field_to_map])
        ...
        if color_vals_transform == 'log':  # log(data)
            ...
            # shift so we don't have any negative values
            data = [(d-np.min(data)) for d in data]
        ...
        edges_with_data = zip(zip(edges1, edges2, keys), data)

    # otherwise perform operations normally
    else:
        edges_with_data = [(e[0], e[1], e[2][field_to_map])
    ...
    if (type(G) == nx.classes.multigraph.MultiGraph) | (type(G) == nx.classes.multidigraph.MultiDiGraph):
        ...
        color_list = [np.multiply(cmap(int(float(
            edge_to_mapField[d]-vmin)/(vmax-vmin)*256)), 256) for d in G.edges(keys=True)]
        color_list = [(int(c[0]), int(c[1]), int(c[2]), alpha)
                      for c in color_list]
        edge_to_color = dict(
            zip(G.edges(keys=True), ['rgba'+str(c) for c in color_list]))

    else:

        edge_to_mapField = dict(edges_with_data)
        color_list = [np.multiply(cmap(
            int(float(edge_to_mapField[d]-vmin)/(vmax-vmin)*256)), 256) for d in G.edges()]
        color_list = [(int(c[0]), int(c[1]), int(c[2]), alpha)
                      for c in color_list]
        edge_to_color = dict(
            zip(G.edges(), ['rgba'+str(c) for c in color_list]))
...
def create_graph_style_file(filename='visJS_html_file_temp',
                            ...
                            node_color_hover_border='#2B7CE9',
                            # color of node when mouse hovers but does not click
                            node_color_hover_background='#D2E5FF',
                            # node does not move in x direction but is still calculated into physics
                            node_fixed_x=False,
                            # node does not move in y direction but is still calculated into physics
                            node_fixed_y=False,
                            ...
                            edge_font_size=20,  # size of label text
                            edge_font_face='ariel',  # font of label text
                            # when givn a color string, a background rectangle of that color will be drawn behind the label
                            edge_font_background='undefined',
                            edge_font_strokeWidth=0,  # stroke drawn around text
    ...
    node_shape_use_border_with_image = stringify_bool(
        node_shape_use_border_with_image)
